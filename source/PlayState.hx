...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['so sus', 0.2], //From 0% to 19%
		['Shit', 0.4], //From 20% to 39%
		['sus', 0.5], //From 40% to 49%
		['Bruh', 0.6], //From 50% to 59%
		['Meh', 0.69], //From 60% to 68%
		['Nice', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Great', 0.9], //From 80% to 89%
		['damn you good as hell', 1], //From 90% to 99%
	...
	var victoryBG:FlxSprite;

	var flashSprite:FlxSprite = new FlxSprite(0, 0).makeGraphic(1280, 720, 0xFFb30000);

	var booptimes:Array<Int> = [920, 924, 928, 956, 966, 994, 1003, 1030, 1040, 1076, 1087, 1096, 1105, 1114, 1124, 1133, 1142, 1151, 1161, 1170, 1178, 1188, 1198, 1672, 1690, 1709, 1728, 1746, 1764, 1783, 1802, 1857];

	var spinArray:Array<Int> = [272, 276, 336, 340, 400, 404, 464, 468, 528, 532, 592, 596, 656, 660, 720, 724, 789, 793, 863, 867, 937, 941, 1012, 1016, 1086, 1090, 1160, 1164, 1531, 1535, 1607, 1611, 1681, 1685, 1754, 1758];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'victory':
				victoryBG = new FlxSprite(-1619,-800).loadGraphic(Paths.image('VictoryBG', 'shared'));
				add(victoryBG);
			case 'parrelel':
		...
		if (boyfriend.curCharacter.toLowerCase() == 'playervictoryblack')
		{
			GameOverSubstate.characterName = 'blackdeathlol';
			GameOverSubstate.deathSoundName = 'blackDeath';
		}

		switch(curStage)
		{
			case 'victory' | 'parrelel' | 'defeat':
	...
	override function stepHit()
	{
		...
		if (lastStep != curStep && curSong == 'Victory')
			{
				switch(curStep)
				{
					case 1984:
						if (curStage.toLowerCase() == 'victory')
						{
							FlxTween.tween(victoryBG, {alpha: 0}, 5);
						}
						FlxTween.tween(dad, {alpha: 0}, 5);
						FlxTween.tween(dad, {x: dad.x - 500}, 5);
				}
			}
			if (lastStep != curStep && curSong == 'endless')
				{
					...
					switch(curStep)
					{
						...
						case 650:
							if (curStage.toLowerCase() == 'victory')
							{
								FlxTween.tween(victoryBG, {alpha: 1}, 10);
							}
						case 909:
